...
export default function Characters() {
  ...
  const { searchTerm, selectedCategory } = useFilters();
  ...
  const filteredData = data.filter((personaje) => {
    const matchesSearch = personaje.name
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const matchesCategory =
      selectedCategory === "all" || personaje.race === selectedCategory;
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4">
        {filteredData.length === 0 ? (
          <div className="col-span-3 text-center text-body p-8">
